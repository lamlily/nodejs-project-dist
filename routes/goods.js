...
Router.post('/getGoods',(req,res)=>{
  ...
  Goods.find()
  .then((data)=>{
  	// 新增获取总条数放在goodslist下；总条数/页面数=每页的条数；
  	obj.total=data.length;
// 按照分页查询到当前页的数据
  	  return Goods.find().limit(Number(pagesize)).skip((Number(page)-1)*Number(pagesize))
      // 得到的是一个promise对象（通过mongoose所有的查询得到的）
  })
  // .then要链式调用不可嵌套使用，因此上面要return，后依然为promise对象；
  // 这个then是获取数据的总条数；
  .then((data)=>{
  	//处理的是第几页的几条数据；
  	obj.goodslist=data
...
Router.post('/delGood',(req,res)=>{
  ...
  //Goods.remove({_id:{$in:array}})//批量删除；比如Goods.remove({_id:{$in:[id1,id2s]});  ｛$in:[]｝批量删除数组的id，把要删除的id组成数组传到后端用{$in:[]};
  Goods.remove({_id:id})
  .then((data)=>{
...
// 模糊查询接口   关键字查询（作为条件放进去[{name:{$regex:'肉'}},{desc:{$regex:'肉'}}]）
Router.post('/findGoodsByKw',(req,res)=>{


  // let id=req.body.keyword;
    // Goods.find({name:{$regex:'肉'}})包含了什么$regex:关键字
   Goods.find({$or:[{name:{$regex:'li'}},{desc:{$regex:'1'}}]})
  .then((data)=>{
